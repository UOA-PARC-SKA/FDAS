...
kernel void fft_4(const uint n_tiles)
{
    ...
    for (uint tile = 0; tile < n_tiles + 2; ++tile) {
        for (uint step = 0; step < 512; ++step) {
            ...
            if (tile >= 2) {
                write_channel_intel(ifft_out[4], buf[tile & 1][step]);
            }

            if (tile < n_tiles) {
                data = read_channel_intel(ifft_in[4]);
            } else {
                data = zeros;
            }
                         ...
                         const uint tmpl_offset_4)
{
    ...
    for (uint pack = 0; pack < 512; ++pack) {
        ...
        float2x4 tmpl_4 = 4 < n_engines_to_use ? templates[tmpl_offset_4 + pack] : zeros;
    ...
    for (uint pack = 0; pack < n_tiles * 512; ++pack) {
        ...
        prods[4] = complex_mult(load, coeffs[4]);
                                 ...
                                 const uint fop_offset)
{
    ...
    for (uint tile = 0; tile < n_tiles + 1; ++tile) {
        for (uint step = 0; step < 512; ++step) {
            ...
            if (tile < n_tiles) {
                float2x4 read = read_channel_intel(ifft_out[4]);
                float4 norm = power_norm(read);
                chunk_buf_0[tile & 1][step] = norm.s0;
                chunk_buf_1[tile & 1][step] = norm.s2;
                chunk_buf_2[tile & 1][step] = norm.s1;
                     ...
                     uint n_bundles)
{
    ...
    for (uint group = 0; group < n_groups; ++group) {
        ...
        for (uint bundle = 0; bundle < n_bundles; ++bundle) {
            ...
            #pragma unroll
            for (uint p = 0; p < 4; ++p) {
                float4 from_fop = read_channel_intel(delay_to_detect[0][p]);
                hsum[p] = from_fop;
            }
            ...
            cand[4] = (hsum[1].s0 > threshold) & tmpl_mask[1];
            cand[5] = (hsum[1].s1 > threshold) & tmpl_mask[1];
            cand[6] = (hsum[1].s2 > threshold) & tmpl_mask[1];
            cand[7] = (hsum[1].s3 > threshold) & tmpl_mask[1];
            cand[8] = (hsum[2].s0 > threshold) & tmpl_mask[2];
            cand[9] = (hsum[2].s1 > threshold) & tmpl_mask[2];
            cand[10] = (hsum[2].s2 > threshold) & tmpl_mask[2];
            cand[11] = (hsum[2].s3 > threshold) & tmpl_mask[2];
            cand[12] = (hsum[3].s0 > threshold) & tmpl_mask[3];
            cand[13] = (hsum[3].s1 > threshold) & tmpl_mask[3];
            cand[14] = (hsum[3].s2 > threshold) & tmpl_mask[3];
            cand[15] = (hsum[3].s3 > threshold) & tmpl_mask[3];
            ...
            if (any_cand) {
                ...
                loc[4] = cand[4] ? encode_location(1, tmpl[1], freq[0]) : invalid_location;
                pwr[4] = cand[4] ? hsum[1].s0 : invalid_power;
                loc[5] = cand[5] ? encode_location(1, tmpl[1], freq[1]) : invalid_location;
                pwr[5] = cand[5] ? hsum[1].s1 : invalid_power;
                loc[6] = cand[6] ? encode_location(1, tmpl[1], freq[2]) : invalid_location;
                pwr[6] = cand[6] ? hsum[1].s2 : invalid_power;
                loc[7] = cand[7] ? encode_location(1, tmpl[1], freq[3]) : invalid_location;
                pwr[7] = cand[7] ? hsum[1].s3 : invalid_power;
                loc[8] = cand[8] ? encode_location(1, tmpl[2], freq[0]) : invalid_location;
                pwr[8] = cand[8] ? hsum[2].s0 : invalid_power;
                loc[9] = cand[9] ? encode_location(1, tmpl[2], freq[1]) : invalid_location;
                pwr[9] = cand[9] ? hsum[2].s1 : invalid_power;
                loc[10] = cand[10] ? encode_location(1, tmpl[2], freq[2]) : invalid_location;
                pwr[10] = cand[10] ? hsum[2].s2 : invalid_power;
                loc[11] = cand[11] ? encode_location(1, tmpl[2], freq[3]) : invalid_location;
                pwr[11] = cand[11] ? hsum[2].s3 : invalid_power;
                loc[12] = cand[12] ? encode_location(1, tmpl[3], freq[0]) : invalid_location;
                pwr[12] = cand[12] ? hsum[3].s0 : invalid_power;
                loc[13] = cand[13] ? encode_location(1, tmpl[3], freq[1]) : invalid_location;
                pwr[13] = cand[13] ? hsum[3].s1 : invalid_power;
                loc[14] = cand[14] ? encode_location(1, tmpl[3], freq[2]) : invalid_location;
                pwr[14] = cand[14] ? hsum[3].s2 : invalid_power;
                loc[15] = cand[15] ? encode_location(1, tmpl[3], freq[3]) : invalid_location;
                pwr[15] = cand[15] ? hsum[3].s3 : invalid_power;
                     ...
                     uint n_bundles)
{
    ...
    for (uint group = 0; group < n_groups; ++group) {
        ...
        for (uint bundle = 0; bundle < n_bundles; ++bundle) {
            ...
            #pragma unroll
            for (uint p = 0; p < 4; ++p) {
                float4 from_prev_hp = read_channel_intel(detect_to_detect[0][p]);
                float4 from_sp = read_channel_intel(delay_to_detect[1][p]);
            ...
            if (any_cand) {
                ...
                pwr[3] = cand[3] ? hsum[0].s3 : invalid_power;
                loc[4] = cand[4] ? encode_location(2, tmpl[1], freq[0]) : invalid_location;
                pwr[4] = cand[4] ? hsum[1].s0 : invalid_power;
                loc[5] = cand[5] ? encode_location(2, tmpl[1], freq[1]) : invalid_location;
                pwr[5] = cand[5] ? hsum[1].s1 : invalid_power;
                loc[6] = cand[6] ? encode_location(2, tmpl[1], freq[2]) : invalid_location;
                pwr[6] = cand[6] ? hsum[1].s2 : invalid_power;
                loc[7] = cand[7] ? encode_location(2, tmpl[1], freq[3]) : invalid_location;
                pwr[7] = cand[7] ? hsum[1].s3 : invalid_power;
                loc[8] = cand[8] ? encode_location(2, tmpl[2], freq[0]) : invalid_location;
                pwr[8] = cand[8] ? hsum[2].s0 : invalid_power;
                loc[9] = cand[9] ? encode_location(2, tmpl[2], freq[1]) : invalid_location;
                pwr[9] = cand[9] ? hsum[2].s1 : invalid_power;
                loc[10] = cand[10] ? encode_location(2, tmpl[2], freq[2]) : invalid_location;
                pwr[10] = cand[10] ? hsum[2].s2 : invalid_power;
                loc[11] = cand[11] ? encode_location(2, tmpl[2], freq[3]) : invalid_location;
                pwr[11] = cand[11] ? hsum[2].s3 : invalid_power;
                loc[12] = cand[12] ? encode_location(2, tmpl[3], freq[0]) : invalid_location;
                pwr[12] = cand[12] ? hsum[3].s0 : invalid_power;
                loc[13] = cand[13] ? encode_location(2, tmpl[3], freq[1]) : invalid_location;
                pwr[13] = cand[13] ? hsum[3].s1 : invalid_power;
                loc[14] = cand[14] ? encode_location(2, tmpl[3], freq[2]) : invalid_location;
                pwr[14] = cand[14] ? hsum[3].s2 : invalid_power;
                loc[15] = cand[15] ? encode_location(2, tmpl[3], freq[3]) : invalid_location;
                pwr[15] = cand[15] ? hsum[3].s3 : invalid_power;
                     ...
                     uint n_bundles)
{
    ...
    for (uint group = 0; group < n_groups; ++group) {
        ...
        for (uint bundle = 0; bundle < n_bundles; ++bundle) {
            ...
            #pragma unroll
            for (uint p = 0; p < 4; ++p) {
                float4 from_prev_hp = read_channel_intel(detect_to_detect[1][p]);
                float4 from_sp = read_channel_intel(delay_to_detect[2][p]);
                hsum[p] = from_prev_hp + from_sp;
            }
            ...
            if (any_cand) {
                ...
                pwr[3] = cand[3] ? hsum[0].s3 : invalid_power;
                loc[4] = cand[4] ? encode_location(3, tmpl[1], freq[0]) : invalid_location;
                pwr[4] = cand[4] ? hsum[1].s0 : invalid_power;
                loc[5] = cand[5] ? encode_location(3, tmpl[1], freq[1]) : invalid_location;
                pwr[5] = cand[5] ? hsum[1].s1 : invalid_power;
                loc[6] = cand[6] ? encode_location(3, tmpl[1], freq[2]) : invalid_location;
                pwr[6] = cand[6] ? hsum[1].s2 : invalid_power;
                loc[7] = cand[7] ? encode_location(3, tmpl[1], freq[3]) : invalid_location;
                pwr[7] = cand[7] ? hsum[1].s3 : invalid_power;
                loc[8] = cand[8] ? encode_location(3, tmpl[2], freq[0]) : invalid_location;
                pwr[8] = cand[8] ? hsum[2].s0 : invalid_power;
                loc[9] = cand[9] ? encode_location(3, tmpl[2], freq[1]) : invalid_location;
                pwr[9] = cand[9] ? hsum[2].s1 : invalid_power;
                loc[10] = cand[10] ? encode_location(3, tmpl[2], freq[2]) : invalid_location;
                pwr[10] = cand[10] ? hsum[2].s2 : invalid_power;
                loc[11] = cand[11] ? encode_location(3, tmpl[2], freq[3]) : invalid_location;
                pwr[11] = cand[11] ? hsum[2].s3 : invalid_power;
                loc[12] = cand[12] ? encode_location(3, tmpl[3], freq[0]) : invalid_location;
                pwr[12] = cand[12] ? hsum[3].s0 : invalid_power;
                loc[13] = cand[13] ? encode_location(3, tmpl[3], freq[1]) : invalid_location;
                pwr[13] = cand[13] ? hsum[3].s1 : invalid_power;
                loc[14] = cand[14] ? encode_location(3, tmpl[3], freq[2]) : invalid_location;
                pwr[14] = cand[14] ? hsum[3].s2 : invalid_power;
                loc[15] = cand[15] ? encode_location(3, tmpl[3], freq[3]) : invalid_location;
                pwr[15] = cand[15] ? hsum[3].s3 : invalid_power;
                     ...
                     uint n_bundles)
{
    ...
    for (uint group = 0; group < n_groups; ++group) {
        ...
        for (uint bundle = 0; bundle < n_bundles; ++bundle) {
            ...
            #pragma unroll
            for (uint p = 0; p < 4; ++p) {
                float4 from_prev_hp = read_channel_intel(detect_to_detect[2][p]);
                float4 from_sp = read_channel_intel(delay_to_detect[3][p]);
                hsum[p] = from_prev_hp + from_sp;
            }
            ...
            if (any_cand) {
                ...
                pwr[3] = cand[3] ? hsum[0].s3 : invalid_power;
                loc[4] = cand[4] ? encode_location(4, tmpl[1], freq[0]) : invalid_location;
                pwr[4] = cand[4] ? hsum[1].s0 : invalid_power;
                loc[5] = cand[5] ? encode_location(4, tmpl[1], freq[1]) : invalid_location;
                pwr[5] = cand[5] ? hsum[1].s1 : invalid_power;
                loc[6] = cand[6] ? encode_location(4, tmpl[1], freq[2]) : invalid_location;
                pwr[6] = cand[6] ? hsum[1].s2 : invalid_power;
                loc[7] = cand[7] ? encode_location(4, tmpl[1], freq[3]) : invalid_location;
                pwr[7] = cand[7] ? hsum[1].s3 : invalid_power;
                loc[8] = cand[8] ? encode_location(4, tmpl[2], freq[0]) : invalid_location;
                pwr[8] = cand[8] ? hsum[2].s0 : invalid_power;
                loc[9] = cand[9] ? encode_location(4, tmpl[2], freq[1]) : invalid_location;
                pwr[9] = cand[9] ? hsum[2].s1 : invalid_power;
                loc[10] = cand[10] ? encode_location(4, tmpl[2], freq[2]) : invalid_location;
                pwr[10] = cand[10] ? hsum[2].s2 : invalid_power;
                loc[11] = cand[11] ? encode_location(4, tmpl[2], freq[3]) : invalid_location;
                pwr[11] = cand[11] ? hsum[2].s3 : invalid_power;
                loc[12] = cand[12] ? encode_location(4, tmpl[3], freq[0]) : invalid_location;
                pwr[12] = cand[12] ? hsum[3].s0 : invalid_power;
                loc[13] = cand[13] ? encode_location(4, tmpl[3], freq[1]) : invalid_location;
                pwr[13] = cand[13] ? hsum[3].s1 : invalid_power;
                loc[14] = cand[14] ? encode_location(4, tmpl[3], freq[2]) : invalid_location;
                pwr[14] = cand[14] ? hsum[3].s2 : invalid_power;
                loc[15] = cand[15] ? encode_location(4, tmpl[3], freq[3]) : invalid_location;
                pwr[15] = cand[15] ? hsum[3].s3 : invalid_power;
                     ...
                     uint n_bundles)
{
    ...
    for (uint group = 0; group < n_groups; ++group) {
        ...
        for (uint bundle = 0; bundle < n_bundles; ++bundle) {
            ...
            #pragma unroll
            for (uint p = 0; p < 4; ++p) {
                float4 from_prev_hp = read_channel_intel(detect_to_detect[3][p]);
                float4 from_sp = read_channel_intel(delay_to_detect[4][p]);
                hsum[p] = from_prev_hp + from_sp;
            }
            ...
            if (any_cand) {
                ...
                pwr[3] = cand[3] ? hsum[0].s3 : invalid_power;
                loc[4] = cand[4] ? encode_location(5, tmpl[1], freq[0]) : invalid_location;
                pwr[4] = cand[4] ? hsum[1].s0 : invalid_power;
                loc[5] = cand[5] ? encode_location(5, tmpl[1], freq[1]) : invalid_location;
                pwr[5] = cand[5] ? hsum[1].s1 : invalid_power;
                loc[6] = cand[6] ? encode_location(5, tmpl[1], freq[2]) : invalid_location;
                pwr[6] = cand[6] ? hsum[1].s2 : invalid_power;
                loc[7] = cand[7] ? encode_location(5, tmpl[1], freq[3]) : invalid_location;
                pwr[7] = cand[7] ? hsum[1].s3 : invalid_power;
                loc[8] = cand[8] ? encode_location(5, tmpl[2], freq[0]) : invalid_location;
                pwr[8] = cand[8] ? hsum[2].s0 : invalid_power;
                loc[9] = cand[9] ? encode_location(5, tmpl[2], freq[1]) : invalid_location;
                pwr[9] = cand[9] ? hsum[2].s1 : invalid_power;
                loc[10] = cand[10] ? encode_location(5, tmpl[2], freq[2]) : invalid_location;
                pwr[10] = cand[10] ? hsum[2].s2 : invalid_power;
                loc[11] = cand[11] ? encode_location(5, tmpl[2], freq[3]) : invalid_location;
                pwr[11] = cand[11] ? hsum[2].s3 : invalid_power;
                loc[12] = cand[12] ? encode_location(5, tmpl[3], freq[0]) : invalid_location;
                pwr[12] = cand[12] ? hsum[3].s0 : invalid_power;
                loc[13] = cand[13] ? encode_location(5, tmpl[3], freq[1]) : invalid_location;
                pwr[13] = cand[13] ? hsum[3].s1 : invalid_power;
                loc[14] = cand[14] ? encode_location(5, tmpl[3], freq[2]) : invalid_location;
                pwr[14] = cand[14] ? hsum[3].s2 : invalid_power;
                loc[15] = cand[15] ? encode_location(5, tmpl[3], freq[3]) : invalid_location;
                pwr[15] = cand[15] ? hsum[3].s3 : invalid_power;
                     ...
                     uint n_bundles)
{
    ...
    for (uint group = 0; group < n_groups; ++group) {
        ...
        for (uint bundle = 0; bundle < n_bundles; ++bundle) {
            ...
            #pragma unroll
            for (uint p = 0; p < 4; ++p) {
                float4 from_prev_hp = read_channel_intel(detect_to_detect[4][p]);
                float4 from_sp = read_channel_intel(delay_to_detect[5][p]);
                hsum[p] = from_prev_hp + from_sp;
            }
            ...
            if (any_cand) {
                ...
                pwr[3] = cand[3] ? hsum[0].s3 : invalid_power;
                loc[4] = cand[4] ? encode_location(6, tmpl[1], freq[0]) : invalid_location;
                pwr[4] = cand[4] ? hsum[1].s0 : invalid_power;
                loc[5] = cand[5] ? encode_location(6, tmpl[1], freq[1]) : invalid_location;
                pwr[5] = cand[5] ? hsum[1].s1 : invalid_power;
                loc[6] = cand[6] ? encode_location(6, tmpl[1], freq[2]) : invalid_location;
                pwr[6] = cand[6] ? hsum[1].s2 : invalid_power;
                loc[7] = cand[7] ? encode_location(6, tmpl[1], freq[3]) : invalid_location;
                pwr[7] = cand[7] ? hsum[1].s3 : invalid_power;
                loc[8] = cand[8] ? encode_location(6, tmpl[2], freq[0]) : invalid_location;
                pwr[8] = cand[8] ? hsum[2].s0 : invalid_power;
                loc[9] = cand[9] ? encode_location(6, tmpl[2], freq[1]) : invalid_location;
                pwr[9] = cand[9] ? hsum[2].s1 : invalid_power;
                loc[10] = cand[10] ? encode_location(6, tmpl[2], freq[2]) : invalid_location;
                pwr[10] = cand[10] ? hsum[2].s2 : invalid_power;
                loc[11] = cand[11] ? encode_location(6, tmpl[2], freq[3]) : invalid_location;
                pwr[11] = cand[11] ? hsum[2].s3 : invalid_power;
                loc[12] = cand[12] ? encode_location(6, tmpl[3], freq[0]) : invalid_location;
                pwr[12] = cand[12] ? hsum[3].s0 : invalid_power;
                loc[13] = cand[13] ? encode_location(6, tmpl[3], freq[1]) : invalid_location;
                pwr[13] = cand[13] ? hsum[3].s1 : invalid_power;
                loc[14] = cand[14] ? encode_location(6, tmpl[3], freq[2]) : invalid_location;
                pwr[14] = cand[14] ? hsum[3].s2 : invalid_power;
                loc[15] = cand[15] ? encode_location(6, tmpl[3], freq[3]) : invalid_location;
                pwr[15] = cand[15] ? hsum[3].s3 : invalid_power;
                     ...
                     uint n_bundles)
{
    ...
    for (uint group = 0; group < n_groups; ++group) {
        ...
        for (uint bundle = 0; bundle < n_bundles; ++bundle) {
            ...
            #pragma unroll
            for (uint p = 0; p < 4; ++p) {
                float4 from_prev_hp = read_channel_intel(detect_to_detect[5][p]);
                float4 from_sp = read_channel_intel(delay_to_detect[6][p]);
                hsum[p] = from_prev_hp + from_sp;
            }
            ...
            if (any_cand) {
                ...
                pwr[3] = cand[3] ? hsum[0].s3 : invalid_power;
                loc[4] = cand[4] ? encode_location(7, tmpl[1], freq[0]) : invalid_location;
                pwr[4] = cand[4] ? hsum[1].s0 : invalid_power;
                loc[5] = cand[5] ? encode_location(7, tmpl[1], freq[1]) : invalid_location;
                pwr[5] = cand[5] ? hsum[1].s1 : invalid_power;
                loc[6] = cand[6] ? encode_location(7, tmpl[1], freq[2]) : invalid_location;
                pwr[6] = cand[6] ? hsum[1].s2 : invalid_power;
                loc[7] = cand[7] ? encode_location(7, tmpl[1], freq[3]) : invalid_location;
                pwr[7] = cand[7] ? hsum[1].s3 : invalid_power;
                loc[8] = cand[8] ? encode_location(7, tmpl[2], freq[0]) : invalid_location;
                pwr[8] = cand[8] ? hsum[2].s0 : invalid_power;
                loc[9] = cand[9] ? encode_location(7, tmpl[2], freq[1]) : invalid_location;
                pwr[9] = cand[9] ? hsum[2].s1 : invalid_power;
                loc[10] = cand[10] ? encode_location(7, tmpl[2], freq[2]) : invalid_location;
                pwr[10] = cand[10] ? hsum[2].s2 : invalid_power;
                loc[11] = cand[11] ? encode_location(7, tmpl[2], freq[3]) : invalid_location;
                pwr[11] = cand[11] ? hsum[2].s3 : invalid_power;
                loc[12] = cand[12] ? encode_location(7, tmpl[3], freq[0]) : invalid_location;
                pwr[12] = cand[12] ? hsum[3].s0 : invalid_power;
                loc[13] = cand[13] ? encode_location(7, tmpl[3], freq[1]) : invalid_location;
                pwr[13] = cand[13] ? hsum[3].s1 : invalid_power;
                loc[14] = cand[14] ? encode_location(7, tmpl[3], freq[2]) : invalid_location;
                pwr[14] = cand[14] ? hsum[3].s2 : invalid_power;
                loc[15] = cand[15] ? encode_location(7, tmpl[3], freq[3]) : invalid_location;
                pwr[15] = cand[15] ? hsum[3].s3 : invalid_power;
                     ...
                     uint n_bundles)
{
    ...
    for (uint group = 0; group < n_groups; ++group) {
        ...
        for (uint bundle = 0; bundle < n_bundles; ++bundle) {
            ...
            #pragma unroll
            for (uint p = 0; p < 4; ++p) {
                float4 from_prev_hp = read_channel_intel(detect_to_detect[6][p]);
                float4 from_sp = read_channel_intel(delay_to_detect[7][p]);
                hsum[p] = from_prev_hp + from_sp;
            }
            ...
            if (any_cand) {
                ...
                pwr[3] = cand[3] ? hsum[0].s3 : invalid_power;
                loc[4] = cand[4] ? encode_location(8, tmpl[1], freq[0]) : invalid_location;
                pwr[4] = cand[4] ? hsum[1].s0 : invalid_power;
                loc[5] = cand[5] ? encode_location(8, tmpl[1], freq[1]) : invalid_location;
                pwr[5] = cand[5] ? hsum[1].s1 : invalid_power;
                loc[6] = cand[6] ? encode_location(8, tmpl[1], freq[2]) : invalid_location;
                pwr[6] = cand[6] ? hsum[1].s2 : invalid_power;
                loc[7] = cand[7] ? encode_location(8, tmpl[1], freq[3]) : invalid_location;
                pwr[7] = cand[7] ? hsum[1].s3 : invalid_power;
                loc[8] = cand[8] ? encode_location(8, tmpl[2], freq[0]) : invalid_location;
                pwr[8] = cand[8] ? hsum[2].s0 : invalid_power;
                loc[9] = cand[9] ? encode_location(8, tmpl[2], freq[1]) : invalid_location;
                pwr[9] = cand[9] ? hsum[2].s1 : invalid_power;
                loc[10] = cand[10] ? encode_location(8, tmpl[2], freq[2]) : invalid_location;
                pwr[10] = cand[10] ? hsum[2].s2 : invalid_power;
                loc[11] = cand[11] ? encode_location(8, tmpl[2], freq[3]) : invalid_location;
                pwr[11] = cand[11] ? hsum[2].s3 : invalid_power;
                loc[12] = cand[12] ? encode_location(8, tmpl[3], freq[0]) : invalid_location;
                pwr[12] = cand[12] ? hsum[3].s0 : invalid_power;
                loc[13] = cand[13] ? encode_location(8, tmpl[3], freq[1]) : invalid_location;
                pwr[13] = cand[13] ? hsum[3].s1 : invalid_power;
                loc[14] = cand[14] ? encode_location(8, tmpl[3], freq[2]) : invalid_location;
                pwr[14] = cand[14] ? hsum[3].s2 : invalid_power;
                loc[15] = cand[15] ? encode_location(8, tmpl[3], freq[3]) : invalid_location;
                pwr[15] = cand[15] ? hsum[3].s3 : invalid_power;
