                             ...
                             global float * restrict detection_power)
{
    ...
    uint __attribute__((numbanks(32))) location_buf[16][4][8];
    float __attribute__((numbanks(32))) power_buf[16][4][8];
    ulong valid[4][8];
    uint next_slot[4][8];
        ...
        for (uint freq = 0; freq < n_frequency_bins; ++freq) {
            ...
            for (uint h = 0; h < 8; ++h) {
                ...
                if (hsum > thrsh[h]) {
                    ...
                    valid[x][h] |= 1l << slot;
                    next_slot[x][h] = (slot == 15) ? 0 : slot + 1;
                }
            }
        }
    }
    ...
    for (uint h = 0; h < 8; ++h) {
        for (uint x = 0; x < 4; ++x) {
            for (uint d = 0; d < 16; ++d) {
                if (valid[x][h] & (1l << d)) {
                    detection_location[h * 64 + x * 16 + d] = location_buf[d][x][h];
                    detection_power[h * 64 + x * 16 + d] = power_buf[d][x][h];
                } else {
                    detection_location[h * 64 + x * 16 + d] = invalid_location;
                    detection_power[h * 64 + x * 16 + d] = invalid_power;
                }
            }
        }
    }
