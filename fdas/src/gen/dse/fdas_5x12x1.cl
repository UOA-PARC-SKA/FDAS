...
channel float preload_to_delay[8][12] __attribute__((depth(0)));
channel float delay_to_detect[8][12] __attribute__((depth(0)));
                      ...
                      )
{
    ...
    for (uint bundle = 0; bundle < n_bundles; ++bundle) {
        ...
        out[6] = cc_of_group_base < 4 ? load[0] : load[1];
        out[7] = cc_of_group_base < 4 ? load[0] : load[1];
        out[8] = load[1];
        out[9] = load[1];
        out[10] = load[1];
        out[11] = load[1];
