...
channel float preload_to_detect[{n_planes}][{n_parallel}][{bundle_sz}] __attribute__((depth(0)));
""")
...
channel float detect_to_detect[{n_planes - 1}][{n_parallel}][{bundle_sz}] __attribute__((depth(0)));
channel uint  detect_to_store_location[{n_planes}][{n_parallel}][{bundle_sz}] __attribute__((depth(0)));
channel float detect_to_store_amplitude[{n_planes}][{n_parallel}][{bundle_sz}] __attribute__((depth(0)));
""")
        for h in range(n_planes):
