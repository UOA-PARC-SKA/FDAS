...
kernel void harmonic_summing(global float * restrict fop,
                             ...
                             global unsigned * restrict detection_location,
...
{
    unsigned location_buf[HMS_N_PLANES][HMS_DETECTION_SZ];
    float amplitude_buf[HMS_N_PLANES][HMS_DETECTION_SZ];
    unsigned long valid[HMS_N_PLANES]; // HMS_DETECTION_SZ must be <= 64!
    int next_slot[HMS_N_PLANES];
    ...
    for (int h = 0; h < HMS_N_PLANES; ++h) {
        ...
        #pragma unroll 1
        for (int d = 0; d < HMS_DETECTION_SZ; ++d) {
            if (v & (1l << d)) {
