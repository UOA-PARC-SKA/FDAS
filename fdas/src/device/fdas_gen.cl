...
channel float2 preload_to_delay[8][6] __attribute__((depth(0)));
channel float2 delay_to_detect[8][6] __attribute__((depth(0)));

channel float2 detect_to_detect[7][6] __attribute__((depth(0)));
                      ...
                      const uint n_channel_bundles)
{
    ...
    for (uint bundle = 0; bundle < n_channel_bundles; ++bundle) {
        ...
        out[0] = load[0];
        out[1] = load[0];
        out[2] = base_row_rem < 2 ? load[0] : load[1];
                      ...
                      const uint n_channel_bundles)
{
    ...
    for (uint bundle = 0; bundle < n_channel_bundles; ++bundle) {
        ...
        out[0] = load[0];
        out[1] = load[0];
        out[2] = base_row_rem < 6 ? load[0] : load[1];
        out[3] = base_row_rem < 6 ? load[0] : load[1];
        out[4] = base_row_rem < 4 ? load[0] : load[1];
        out[5] = base_row_rem < 4 ? load[0] : load[1];
...
{
    uint location_buffer[64][12];
    float amplitude_buffer[64][12];
    ...
    for (uint d = 0; d < 64; ++d) {
        ...
        #pragma unroll
        for (uint x = 0; x < 12; ++x) {
            detection_location[0 + d * 12 + x] = is_valid ? location_buffer[d][x] : invalid_location;
            detection_amplitude[0 + d * 12 + x] = is_valid ? amplitude_buffer[d][x] : invalid_amplitude;
        }
    }
}
                     ...
                     uint n_channel_bundles)
{
    ...
    for (uint d = 0; d < 64; ++d) {
        ...
        #pragma unroll
        for (uint x = 0; x < 12; ++x) {
            detection_location[768 + d * 12 + x] = is_valid ? location_buffer[d][x] : invalid_location;
            detection_amplitude[768 + d * 12 + x] = is_valid ? amplitude_buffer[d][x] : invalid_amplitude;
        }
    }
}
                     ...
                     uint n_channel_bundles)
{
    ...
    for (uint d = 0; d < 64; ++d) {
        ...
        #pragma unroll
        for (uint x = 0; x < 12; ++x) {
            detection_location[1536 + d * 12 + x] = is_valid ? location_buffer[d][x] : invalid_location;
            detection_amplitude[1536 + d * 12 + x] = is_valid ? amplitude_buffer[d][x] : invalid_amplitude;
        }
    }
}
                     ...
                     uint n_channel_bundles)
{
    ...
    for (uint d = 0; d < 64; ++d) {
        ...
        #pragma unroll
        for (uint x = 0; x < 12; ++x) {
            detection_location[2304 + d * 12 + x] = is_valid ? location_buffer[d][x] : invalid_location;
            detection_amplitude[2304 + d * 12 + x] = is_valid ? amplitude_buffer[d][x] : invalid_amplitude;
        }
    }
}
                     ...
                     uint n_channel_bundles)
{
    ...
    for (uint d = 0; d < 64; ++d) {
        ...
        #pragma unroll
        for (uint x = 0; x < 12; ++x) {
            detection_location[3072 + d * 12 + x] = is_valid ? location_buffer[d][x] : invalid_location;
            detection_amplitude[3072 + d * 12 + x] = is_valid ? amplitude_buffer[d][x] : invalid_amplitude;
        }
    }
}
                     ...
                     uint n_channel_bundles)
{
    ...
    for (uint d = 0; d < 64; ++d) {
        ...
        #pragma unroll
        for (uint x = 0; x < 12; ++x) {
            detection_location[3840 + d * 12 + x] = is_valid ? location_buffer[d][x] : invalid_location;
            detection_amplitude[3840 + d * 12 + x] = is_valid ? amplitude_buffer[d][x] : invalid_amplitude;
        }
    }
}
                     ...
                     uint n_channel_bundles)
{
    ...
    for (uint d = 0; d < 64; ++d) {
        ...
        #pragma unroll
        for (uint x = 0; x < 12; ++x) {
            detection_location[4608 + d * 12 + x] = is_valid ? location_buffer[d][x] : invalid_location;
            detection_amplitude[4608 + d * 12 + x] = is_valid ? amplitude_buffer[d][x] : invalid_amplitude;
        }
    }
}
                     ...
                     uint n_channel_bundles)
{
    ...
    for (uint d = 0; d < 64; ++d) {
        ...
        #pragma unroll
        for (uint x = 0; x < 12; ++x) {
            detection_location[5376 + d * 12 + x] = is_valid ? location_buffer[d][x] : invalid_location;
            detection_amplitude[5376 + d * 12 + x] = is_valid ? amplitude_buffer[d][x] : invalid_amplitude;
        }
    }
}
