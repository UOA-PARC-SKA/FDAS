...
{
    uint  __attribute__((numbanks(4))) location_buf[4][8];
    float __attribute__((numbanks(4))) amplitude_buf[4][8];
    ...
    for (uint d = 0; d < 4; ++d) {
        ...
        for (uint f = 0; f < 8; ++f) {
            if (valid[f] & (1 << d)) {
                detection_location[0 * 32 + d * 4 + f] = location_buf[d][f];
                detection_amplitude[0 * 32 + d * 4 + f] = amplitude_buf[d][f];
            }
        }
    }
}
                      ...
                      global float * restrict detection_amplitude)
{
    ...
    for (uint d = 0; d < 4; ++d) {
        ...
        for (uint f = 0; f < 8; ++f) {
            if (valid[f] & (1 << d)) {
                detection_location[1 * 32 + d * 4 + f] = location_buf[d][f];
                detection_amplitude[1 * 32 + d * 4 + f] = amplitude_buf[d][f];
            }
        }
    }
}
                      ...
                      global float * restrict detection_amplitude)
{
    ...
    for (uint d = 0; d < 4; ++d) {
        ...
        for (uint f = 0; f < 8; ++f) {
            if (valid[f] & (1 << d)) {
                detection_location[2 * 32 + d * 4 + f] = location_buf[d][f];
                detection_amplitude[2 * 32 + d * 4 + f] = amplitude_buf[d][f];
            }
        }
    }
}
                      ...
                      global float * restrict detection_amplitude)
{
    ...
    for (uint d = 0; d < 4; ++d) {
        ...
        for (uint f = 0; f < 8; ++f) {
            if (valid[f] & (1 << d)) {
                detection_location[3 * 32 + d * 4 + f] = location_buf[d][f];
                detection_amplitude[3 * 32 + d * 4 + f] = amplitude_buf[d][f];
            }
        }
    }
}
                      ...
                      global float * restrict detection_amplitude)
{
    ...
    for (uint d = 0; d < 4; ++d) {
        ...
        for (uint f = 0; f < 8; ++f) {
            if (valid[f] & (1 << d)) {
                detection_location[4 * 32 + d * 4 + f] = location_buf[d][f];
                detection_amplitude[4 * 32 + d * 4 + f] = amplitude_buf[d][f];
            }
        }
    }
}
                      ...
                      global float * restrict detection_amplitude)
{
    ...
    for (uint d = 0; d < 4; ++d) {
        ...
        for (uint f = 0; f < 8; ++f) {
            if (valid[f] & (1 << d)) {
                detection_location[5 * 32 + d * 4 + f] = location_buf[d][f];
                detection_amplitude[5 * 32 + d * 4 + f] = amplitude_buf[d][f];
            }
        }
    }
}
                      ...
                      global float * restrict detection_amplitude)
{
    ...
    for (uint d = 0; d < 4; ++d) {
        ...
        for (uint f = 0; f < 8; ++f) {
            if (valid[f] & (1 << d)) {
                detection_location[6 * 32 + d * 4 + f] = location_buf[d][f];
                detection_amplitude[6 * 32 + d * 4 + f] = amplitude_buf[d][f];
            }
        }
    }
}
                      ...
                      global float * restrict detection_amplitude)
{
    ...
    for (uint d = 0; d < 4; ++d) {
        ...
        for (uint f = 0; f < 8; ++f) {
            if (valid[f] & (1 << d)) {
                detection_location[7 * 32 + d * 4 + f] = location_buf[d][f];
                detection_amplitude[7 * 32 + d * 4 + f] = amplitude_buf[d][f];
            }
        }
    }
}
